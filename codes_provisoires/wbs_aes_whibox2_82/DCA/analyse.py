...
def filter_good(next):
    ...
    for key, val in next.items():
        if len(val) == 2:
            ...
            # eprint(list(val.items())[0])
            # eprint(list(val.items())[1])
            total = list(val.items())[1][1] + list(val.items())[0][1]
            rating = list(val.items())[0][1] / total

            twonext.append((abs(0.5 - rating), total, val, key))

    return list(filter(lambda x: x[0] < 0.2 and x[1] > 9, twonext))


def goodforall(rounds):
    next_rounds = defaultdict(lambda: defaultdict(int))
    rounds_iter = iter(rounds)
    prev = next(rounds_iter)
    for number in rounds_iter:
        next_rounds[prev][number] += 1
        prev = number
    return next_rounds
