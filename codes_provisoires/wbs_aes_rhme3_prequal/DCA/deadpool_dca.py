...
def findbin(keyword):
    n=len(glob.glob('traces_bin/trace_%s_*.bin' % keyword))
    assert n > 0
    traces_meta={}
    min_size=None
    iblock_available=True
    oblock_available=True
    for filename in glob.glob('traces_bin/trace_%s_*.bin' % keyword):
        i,iblock,oblock=filename[len('traces_bin/trace_%s_' % keyword):-len('.bin')].split('_')
        if iblock!='na':
...
""" % (nsamples, \
   ...
   config['memory'], config['top'])
            with open('%s_%i_%i.%sconfig' % (keyword, ntraces, nsamples, configname), 'wb') as fileconfig:
                fileconfig.write(content.encode('ascii'))
...
def bin2trs(keyword=None, keywords=None, delete_bin=True):
    ...
    for keyword in keywords:
        ...
        with open('%s_%i_%i.trs' % (keyword, ntraces, nsamples), 'wb') as trs:
            ...
            trs.write(b'\x43\x01' + bytes([struct.calcsize('<B')]))
            # Length of crypto data
            # trs.write(b'\x44\x02' + struct.pack('<H', blocksize*iblock_available+blocksize*oblock_available))
            trs.write(b'\x44\x02' + struct.pack('<H', int(blocksize*bool(iblock_available) + blocksize*bool(oblock_available))))
        ...
        with open(filename) as info:
            for i, line in enumerate(info):
                if i+1 == ievent:
                    ...
                    try:
                        output=bytes.fromhex(subprocess.check_output(['addr2line', '-e', target, '0x%X'%ins_addr]))
...
class Tracer(object):
    ...
    def _trace_dump(self):
        ...
        if oblock is not None:
            ...
            oblockstr='na'
        for f in self.filters:
            with open('traces_bin/trace_%s_%04i_%s_%s.bin'
                  % (f.keyword, n, iblockstr, oblockstr), 'wb') as trace:
                #print([type(x) for x in self._trace_data[f.keyword]])
                #print(f.keyword)
                #print(f.extract_fmt)
                trace.write(b''.join([struct.pack(f.extract_fmt, x) for x in self._trace_data[f.keyword]]))
            if f.record_info:
                with open('traces_bin/trace_%s_%s_%s.info'
                    ...
                    for mem_mode, item, ins_addr, mem_addr, mem_size, mem_data in self._trace_info[f.keyword]:
                        trace.write(("[%s] %7i %16X %16X %2i %0*X\n" % (mem_mode, item, ins_addr, mem_addr, mem_size, 2*mem_size, mem_data)).encode('ascii'))
                f.record_info=False
        ...
        with open(self.tmptracefile, 'r') as trace:
            for line in iter(trace.readline, ''):
                if len(line) > 2 and (line[1]=='R' or line[1]=='W'):
                    m=re.search(r'\[(.)\]\s*([0-9]+)\s*(0x[0-9a-fA-F]+).*?(0x[0-9a-fA-F]+)\s+size=\s*([0-9]+)\s+value=\s*(.*)', line, re.VERBOSE)
...
class TracerGrind(Tracer):
    ...
    def get_trace(self, n, iblock):
        ...
        output=self._exec(cmd_list, input_stdin)
        oblock=self.processoutput(output, self.blocksize)
        output=subprocess.check_output("texttrace %s >(grep '^.M' > %s)" % (self.tmptracefile+'.grind', self.tmptracefile), shell=True, executable='/bin/bash').decode()
        if not self.debug:
    ...
    def run_once(self, iblock=None, tracefile=None):
        ...
        output=self._exec(cmd_list, input_stdin, debug=True)
        output=subprocess.check_output("texttrace %s %s" % (tracefile+'.grind',tracefile), shell=True).decode()
...
"""    if not _out:
        for b in range(256):
            n = b
            o = bytes((n >> i & 1 for i in range(8)))
            _out[bytes([b])] = o
